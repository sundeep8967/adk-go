...
type Config struct {
	...
	// tail retention would summarize and it never fires. Enable one.
	//
...
func (r *Runner) compactAfterInvocation(ctx context.Context, storedSession session.Session, ictx agent.InvocationContext) error {
	...
	// was delivered.
	if latest, err := r.reloadSession(ctx, storedSession); err != nil {
		...
		if err := r.sessionService.AppendEvent(ctx, current, repair); err != nil {
